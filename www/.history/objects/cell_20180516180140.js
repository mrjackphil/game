...
class Cell{
    ...
    checkMatches(object){
        ...
        let horiz_matches = cellHandler.objects //Check horisontals
                        .filter(i=>i.color === object.color)//all the same color
                        .filter(i=>(i.column === object.column - 1 || 
                                    i.column === object.column + 1)&& 
                                    i.row == object.row)
        let vert_matches = cellHandler.objects //Check vertical matches
                        .filter(i=>i.color === object.color)//all the same color
                        .filter(i=>(i.row === object.row - 1 || 
                                    i.row === object.row + 1)&& 
                                    i.column == object.column);
        let all_matches = [];
        if (horiz_matches.length > 1) all_matches.push(horiz_matches);
        if (vert_matches.length > 1) all_matches.push(vert_matches);
        if (all_matches.length > 0) return all_matches
    }
}