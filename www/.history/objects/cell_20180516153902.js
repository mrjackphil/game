...
class Cell{
    ...
    choose(){
        let tweenCells = cellHandler.objects.filter(i=>i.object.tween);
        for (let i in tweenCells) {
            tweenCells[i].object.tween.stop();
            tweenCells[i].object.tween.remove();
        }

        cellHandler.objects.scaleX = 1;
        cellHandler.objects.scaleY = 1;

        this.object.tween = this.game.tweens.add({
            targets: [this.object],
            scaleX: 2,
            scaleY: 2,
            yoyo: true,
