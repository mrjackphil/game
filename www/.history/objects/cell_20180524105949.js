...
class Cell{
    ...
    checkMatches(object, array){
        ...
        for (let c in horizontal){
            if (horizontal[c].match !== true)(this.checkMatches(horizontal[c]));
        }
        all = cellHandler.objects.filter(i=>i.match === true);
        for (var c in cellHandler.objects){
            cellHandler.objects[c] = false;
        }
        return all;
    }
}